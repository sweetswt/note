## [基础](https://developer.mozilla.org/zh-CN/docs/Web/JavaScript/Guide/Grammar_and_types#基础)
...
在这个数组中，有两个已被赋值的元素，和一个空元素（`fish[0`]是`Lion`，`fish[1]`是`undefined`，而`fish[2]`是`Angel`；此时数组的长度属性`fish.length=3`)。
